...
class MessagesControllerTest < ActionDispatch::IntegrationTest
  def sign_in(uid)
    ...
    fake_cert.subject = OpenSSL::X509::Name.parse "/CN=#{uid}"
    { 'puma.peercert' => fake_cert }
  end

  test 'can read messages' do
    account = create_account('default_test_user')
    generate_queue_messages(account.queue, 10)
    env = sign_in('default_test_user')
    get '/messages', params: {}, env: env, as: :json
  ...
  test 'can mark message read ' do
    ...
    post "/messages/#{m[:id]}/read", env: env, as: :json
    assert_response :success
  ...
  test 'can bulk mark messages read ' do
    ...
    env = sign_in('default_test_user')
    post '/messages/read', params: { ids: ids }, env: env, as: :json
    assert_response :success
    account.queue.all.each do |message|
  ...
  test 'can limit messages ' do
    ...
    env = sign_in('default_test_user')
    get '/messages', params: { limit: 5 }, env: env, as: :json
    assert_response :success
